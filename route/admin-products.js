...
router.post('/add-products', forUSER, urlencodedParser,[
    check('name', 'Name cant be less than 2')
    .exists()
    .isLength({min:2}),
    ...
    if(!errors.isEmpty()){
        ...
        // return res.status(422).json(errors.array())
            ...
            var uploadpath = __dirname + '/' +'../' +'views'+'/'+'admin'+'/'+ img.name
            img.mv(uploadpath, function(err){
                    if(err) {
                        return res.status(500).send(err);
                    }else{
                        var products = "INSERT INTO products values(null,'"+ title+"', '"+ description +"', '"+ catagory +"', '"+ price+"',  '"+ img.name +"')";
